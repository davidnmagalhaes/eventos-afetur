...
$result_curso = "SELECT * FROM evn_pedidos WHERE status != 3 && status != 12";
...
<div class="row">
<div class="col">
 <h1>PEDIDOS <strong>PENDENTES</strong></h1>
 <form action="pesquisar-pedido-pago.php" method="post">
 <div class="input-group">
        <div class="input-group-prepend">
          <div class="input-group-text"><i class="fas fa-search"></i></div>
        </div>
        <input type="text" name="pesquisar" id="pesquisar" class="form-control" id="inlineFormInputGroupUsername" placeholder="Digite o número, nome ou e-mail do pedido...">
 ...
 <div class="row">
				<?php while($rows_cursos = mysqli_fetch_assoc($resultado_cursos)){ ?>
						...
						<table class="table">
						  <thead class="thead-dark">
							<tr>
							  <th scope="col" colspan="2"># <?php echo $rows_cursos['idpedido']; ?> | <?php if($rows_cursos['status'] == "PAGAMENTO APROVADO"){echo "<span style='color: #63f77c;'>".$rows_cursos['status']."</span>";}elseif($rows_cursos['status'] == "CANCELADO"){echo "<span style='color: #ea5656;'>".$rows_cursos['status']."</span>";}elseif($rows_cursos['status'] == "PAGAMENTO TERCEIRO"){echo "<span style='color: #fbff00;'>".$rows_cursos['status']."</span>";}else{echo "<span style='color: #f7a763;'>".$rows_cursos['status']."</span>";}; ?><?php if($_SESSION['nivel'] == 3){?> <a href="edt-pedido-pendente.php?id=<?php echo $rows_cursos['idpedido'];?>" class="btn btn-info" role="button"><i class="fas fa-edit"></i> Editar</a><?php } ?></th>
			...
			<nav class="text-center">
				<ul class="pagination justify-content-center">
						...
						<li class="page-item">
							<a class="page-link" href="pedidos-pendentes.php?pagina=<?php echo $pagina_anterior; ?>" aria-label="Previous">
					...
					for($i = 1; $i < 5 + 1; $i++){ ?>
						<li class="page-item"><a class="page-link" href="pedidos-pendentes.php?pagina=<?php echo $i; ?>"><?php echo $i; ?></a></li>
					<?php } ?>
					
						<?php
						if($pagina_posterior <= $num_pagina){ ?>
						<li class="page-item">
							<a class="page-link" href="pedidos-pendentes.php?pagina=<?php echo $pagina_posterior; ?>" aria-label="Previous">
