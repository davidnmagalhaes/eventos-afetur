...
$sql .= "FROM evn_log_tentativa WHERE ip = ? and DATE_FORMAT(data_hora,'%Y-%m-%d') = ? AND bloqueado = ?";
...
$sql = 'SELECT cod_usuario, nome, senha, email, funcao, imagem FROM evn_usuario WHERE email = ? AND status = ? LIMIT 1';
$stm = $conexao->prepare($sql);
...
if(!empty($retorno) && password_verify($senha, $retorno->senha)):
	...
	$sql = 'INSERT INTO evn_log_tentativa (ip, email, senha, origem, bloqueado) VALUES (?, ?, ?, ?, ?)';
	$stm = $conexao->prepare($sql);
	$stm->bindValue(1, $_SERVER['SERVER_ADDR']);
...
if ($_SESSION['logado'] == 'SIM'):
	...
	//echo json_encode($retorno);
	echo "<script>javascript:window.location='../eventos.php'</script>";
	exit();
else:
